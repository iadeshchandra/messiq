...
class PdfService {
  ...
  static Future<File> generateMemberLedgerPdf({
          ...
          return [
            ...
            if (deposits.isNotEmpty) ...[
              ...
              pw.TableHelper.fromTextArray(
                ...
                data: deposits.map((d) => [d.date.toString().split(' ')[0], 'Tk ${d.amount.toStringAsFixed(0)}']).toList(),
              ),
              pw.SizedBox(height: 24),
            ],
            ...
            // Meals Table
            if (meals.isNotEmpty) ...[
              pw.Text('Meals Logged', style: pw.TextStyle(fontSize: 14, fontWeight: pw.FontWeight.bold, color: textDark)),
              pw.SizedBox(height: 8),
              pw.TableHelper.fromTextArray(
                headers: ['Date', 'Meal Count'],
                headerStyle: pw.TextStyle(color: PdfColors.white, fontWeight: pw.FontWeight.bold, fontSize: 10),
                headerDecoration: pw.BoxDecoration(color: PdfColors.orange600),
                cellStyle: pw.TextStyle(color: textDark, fontSize: 10),
                data: meals.map((m) => [m.date.toString().split(' ')[0], '${m.memberMeals[member.uid]?.toStringAsFixed(1)}']).toList(),
              ),
              pw.SizedBox(height: 24),
            ],

            // Expenses Added Table
            if (expenses.isNotEmpty) ...[
              pw.Text('Expenses Added to Mess', style: pw.TextStyle(fontSize: 14, fontWeight: pw.FontWeight.bold, color: textDark)),
              pw.SizedBox(height: 8),
              pw.TableHelper.fromTextArray(
                ...
                headerDecoration: pw.BoxDecoration(color: primaryColor),
                cellStyle: pw.TextStyle(color: textDark, fontSize: 10),
                data: expenses.map((e) => [e.date.toString().split(' ')[0], e.type, e.description, 'Tk ${e.amount.toStringAsFixed(0)}']).toList(),
              ),
            ],
          ];
        },
          ...
          return pw.Column(
            children: [
              ...
              pw.Row(
                mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
                children: [
                  pw.Text('Generated by MessIQ - Member Activity Ledger', style: pw.TextStyle(color: PdfColors.grey500, fontSize: 10)),
                  pw.Text('Page ${context.pageNumber} of ${context.pagesCount}', style: pw.TextStyle(color: PdfColors.grey500, fontSize: 10)),
                ],
              ),
            ],
    ...
    final output = await getTemporaryDirectory();
    final file = File('${output.path}/MessIQ_Ledger_${member.name.replaceAll(' ', '_')}_${DateTime.now().millisecondsSinceEpoch}.pdf');
    await file.writeAsBytes(await pdf.save());
    return file;
  }
